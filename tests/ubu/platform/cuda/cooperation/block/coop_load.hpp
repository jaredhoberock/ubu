...
void test_coop_load(int num_warps_per_block, int num_elements_per_block)
{
                              ...
                              [=](ubu::int2 coord, auto ws)
  {
    ...
    int* result_ptr = result_view.data() + max_num_elements_per_thread * coord.x;
  ...
  {
    std::cerr << "test_coop_load(" << num_elements_per_block << ", " << num_elements_per_block << ") failed" << std::endl;
    assert(false);
  }
#endif
}
...
void test_coop_load()
{
  ...
  for(int num_warps_per_block = 1; num_warps_per_block <= max_num_warps_per_block; num_warps_per_block *= 2)
  {
    ...
    for(int num_elements_per_block = 1; num_elements_per_block <= max_num_elements_per_block; num_elements_per_block *= 2)
    {
      ...
      test_coop_load<max_num_elements_per_thread>(num_warps_per_block, num_elements_per_block);
    }
  }
}
