...
void do_coop_load_columns(int block_size, ubu::span_like auto input, ubu::span_like auto result, ubu::layout_like auto matrix_layout)
{
  using namespace ubu;
                              ...
                              [=](ubu::int2 coord, auto ws)
  {
    ...
    // get the matching column of the result
    auto result_slice = slice(result_view, std::pair(_, id(block)));

    for(int i = 0; i < thread_values.size(); ++i)
    {
      result_slice[i] = thread_values[i];
    }
  });
}

template<int max_num_elements_per_thread>
void test_coop_load_columns(int num_warps_per_block, int num_elements_per_block)
{
...

  {
    ...
    {
      std::cerr << "test_coop_load_columns<" << max_num_elements_per_thread << ">(" << num_warps_per_block << ", " << num_elements_per_block << ") accelerated case failed" << std::endl;
      assert(false);
    }
  }
...

  {
    ...
    if(not std::equal(expected.begin(), expected.end(), result.begin()))
    {
      std::cerr << "test_coop_load_columns<" << max_num_elements_per_thread << ">(" << num_warps_per_block << ", " << num_elements_per_block << ") non-accelerated case failed" << std::endl;
      assert(false);
    }
  }
#endif
...
void test_coop_load_columns()
{
  ...
  for(int num_warps_per_block = 1; num_warps_per_block <= max_num_warps_per_block; num_warps_per_block *= 2)
  {
    ...
    for(int num_elements_per_block = 1; num_elements_per_block <= max_num_elements_per_block; num_elements_per_block *= 2)
    {
      ...
      test_coop_load_columns<max_num_elements_per_thread>(num_warps_per_block, num_elements_per_block);
    }
  }
}
...
void test_coop_load_columns()
{
  ...
  test_coop_load_columns<0>();
  test_coop_load_columns<1>();
  test_coop_load_columns<3>();
  test_coop_load_columns<7>();
  test_coop_load_columns<11>();
  test_coop_load_columns<16>();
}
