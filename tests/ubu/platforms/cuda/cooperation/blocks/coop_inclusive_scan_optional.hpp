...
void test(int block_size, int num_holes)
{
                              ...
                              [=](ubu::int2 coord, auto ws)
  {
    ...
    std::optional result = coop_inclusive_scan(block, value, std::plus{});
  ...
  {
    std::cerr << "test(" << block_size << ", " << num_holes << ") failed" << std::endl;
    assert(false);
  }
#endif
}

void test_coop_inclusive_scan_optional()
{
