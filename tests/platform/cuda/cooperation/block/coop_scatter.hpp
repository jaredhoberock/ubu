...
void test_correctness(int block_size, int num_valid)
{
  ...
  // randomly create destination indices
  device_vector<int> destinations = choose_unique_numbers(block_size, block_size);
  std::span destinations_view(destinations.data(), destinations.size());
                              ...
                              [=](ubu::int2 coord, auto ws)
  {
    ...
    int destination = destinations_view[coord.x];
    result_view[coord.x] = coop_scatter(block, value, destination);
  });

  std::vector<std::optional<int>> expected(block_size);
  for(int i = 0; i < block_size; ++i)
  {
    expected[destinations[i]] = input[i];
  }
