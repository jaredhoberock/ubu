// circle --verbose -O3 -std=c++20 -I. -sm_80 load_scan_store_after.cpp -lcudart -lfmt -o load_scan_store_after
...
constexpr ubu::layout_like_of_rank<3> auto layout_for_scan(std::size_t n)
{
  using namespace ubu;
...
ubu::cuda::event load_scan_store_after(ubu::cuda::device_executor gpu, ubu::cuda::device_allocator<std::byte> alloc, const ubu::cuda::event& before, I input, R result, BinaryFunction op)
{
  using namespace ubu;
  using T = tensor_element_t<I>;

  layout_like_of_rank<3> auto layout = layout_for_scan(std::size(input));
  ...
  // circle build 208 -sm_80: 39 registers
  // 404.951 GB/s ~ 93% peak bandwidth on RTX 3070
  // 685.210 GB/s ~ 91% peak bandwidth on RTX A5000
                                          ...
                                          [=](ubu::int2 idx, auto workspace)
  {
    ...
    matrix_like auto input_tile  = slice(input_tiles, std::tuple(_, _, block_idx));
    ...
    std::optional thread_carry_in = coop_exclusive_scan(block, std::optional<T>(), thread_values.maybe_back(), op);
...
void test_load_scan_store_after(std::size_t n)
{
  ...
  load_scan_store_after(ex, alloc, before, std::span(input), std::span(result), plus()).wait();
...
double test_performance(std::size_t size, std::size_t num_trials)
{
  ...
  // warmup
  load_scan_store_after(ex, alloc, before, input_view, result_view, std::plus{});
...
performance_expectations_t load_scan_store_after_expectations = {
  {"NVIDIA GeForce RTX 3070", {0.92, 0.94}},
