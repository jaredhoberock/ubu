...
#include "stride.hpp"
#include <algorithm>
#include <ubu/platform/cuda/shmalloc.hpp>
#include <cmath>
#include <concepts>
#include <iterator>
#include <optional>
...
// XXX there may be a better way to broadcast the result
//     without consuming additional __shared__ memory
//     because the pointer already lives somewhere in the __shared__ heap
template<class = void>
void* block_shmalloc(int thread_idx, std::size_t num_bytes)
{
  __shared__ void* result;

  if(thread_idx == 0)
  {
    result = ubu::cuda::shmalloc(num_bytes);
  }
...
template<class = void>
void block_shfree(int thread_idx, void* ptr)
{
  if(thread_idx == 0)
  {
    ubu::cuda::shfree(ptr);
...
// XXX eliminate block_size parameter
...
T block_reduce(int thread_idx, T value, int num_values, F binary_op)
{
  ...
  T* s_partial_results = reinterpret_cast<T*>(block_shmalloc(thread_idx, num_warps * sizeof(T)));
  ...
  block_shfree(thread_idx, s_partial_results);
...
void shmalloc_reduce_tiles_kernel(int block_idx, int thread_idx, V tiles, O output, F op)
{
  using namespace std::ranges;
