...
constexpr coordinate auto lexicographic_lift_impl(const C& dividend, const S&)
{
...
constexpr coordinate auto lexicographic_lift_impl(const C1& dividend, const C2& divisor)
{
...
constexpr coordinate auto lexicographic_lift_impl(const C1& dividend, const C2& divisor)
{
  return detail::tuple_fold_right(std::make_pair(dividend, std::make_tuple()), divisor, [](auto prev, auto s)
  {
    ...
    auto [quotient, remainder] = detail::lexicographic_lift_impl(prev_quotient, s);
...
constexpr coordinate auto lexicographic_lift_impl(const C1& dividend, const C2& divisor)
{
  ...
  {
    return detail::lexicographic_lift_impl(dividend, divisor);
  }));
}
...
template<coordinate C1, coordinate C2>
  requires weakly_congruent<C1,C2>
constexpr congruent<C2> auto lexicographic_lift(const C1& dividend, const C2& divisor)
{
  return detail::lexicographic_lift_impl(dividend, divisor).second;
}
