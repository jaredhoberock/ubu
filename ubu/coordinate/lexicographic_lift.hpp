...
constexpr coordinate auto lexicographic_lift(const C& dividend, const S&)
{
...
constexpr coordinate auto lexicographic_lift(const C1& dividend, const C2& divisor)
{
...
constexpr coordinate auto lexicographic_lift(const C1& dividend, const C2& divisor)
{
  return detail::tuple_fold_right(std::make_pair(dividend, std::make_tuple()), divisor, [](auto prev, auto s)
  {
    ...
    auto [quotient, remainder] = lexicographic_lift(prev_quotient, s);
...
constexpr coordinate auto lexicographic_lift(const C1& dividend, const C2& divisor)
{
  ...
  {
    return lexicographic_lift(dividend, divisor);
  }));
}
