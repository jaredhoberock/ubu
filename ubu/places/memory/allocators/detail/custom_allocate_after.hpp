...
concept has_allocate_after_member_function_template = requires(A alloc, B before, S shape)
{
  ...
  std::forward<A>(alloc).template allocate_after<T>(std::forward<B>(before), std::forward<S>(shape));
};
...
concept has_allocate_after_member_function = requires(A alloc, B before, S shape)
{
  ...
  // XXX this should check that the result is a pair<E,pointer>
  std::forward<A>(alloc).allocate_after(std::forward<B>(before), std::forward<S>(shape));
};
...
concept has_allocate_after_free_function = requires(A alloc, B before, S shape)
{
  ...
  // XXX this should check that the result is a pair<E,pointer>
  allocate_after(std::forward<A>(alloc), std::forward<B>(before), std::forward<S>(shape));
};
...
constexpr auto custom_allocate_after(A&& alloc, B&& before, S&& shape)
{
