...
constexpr std::regular_invocable auto make_default_bulk_execute_after_invocable(const S& shape, std::size_t workspace_size, F&& function)
{
...
template<coordinate S, std::regular_invocable<S, std::span<std::byte>> F>
using default_bulk_execute_after_invocable_t = decltype(make_default_bulk_execute_after_invocable(std::declval<S>(), std::declval<std::size_t>(), std::declval<F>()));
...
executor_happening_t<E> default_bulk_execute_after(E&& ex, H&& before, const S& shape, std::size_t workspace_size, F&& function)
{
  // create an invocable to represent the kernel
  std::regular_invocable auto kernel = make_default_bulk_execute_after_invocable(shape, workspace_size, std::forward<F>(function));
...
concept has_default_bulk_execute_after = requires(E ex, B before, S shape, W workspace_shape, F f)
{
  { default_bulk_execute_after(std::forward<E>(ex), std::forward<B>(before), std::forward<S>(shape), std::forward<W>(workspace_shape), std::forward<F>(f)) } -> happening;
};
