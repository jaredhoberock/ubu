...
class dispatch_bulk_execute_after
{
  public:
    ...
    template<class Ex, class Ev, class F>
      requires has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int2,F&&>
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int2 grid_shape, F&& function) const
    {
      return std::forward<Ex>(executor).bulk_execute_after(std::forward<Ev>(event), grid_shape, std::forward<F>(function));
    }

    template<class Ex, class Ev, class F>
      requires has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int3,F&&>
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int3 grid_shape, F&& function) const
    {
      return std::forward<Ex>(executor).bulk_execute_after(std::forward<Ev>(event), grid_shape, std::forward<F>(function));
    }

    template<class Ex, class Ev, class F>
      requires has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int4,F&&>
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int4 grid_shape, F&& function) const
    {
      return std::forward<Ex>(executor).bulk_execute_after(std::forward<Ev>(event), grid_shape, std::forward<F>(function));
    }
    ...
    // like this: bulk_execute_after(ex, before, {x,y,z}, func)
    template<class Ex, class Ev, class F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int2,F&&> and has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int2,F&&>)
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int2 grid_shape, F&& function) const
    {
    ...
    template<class Ex, class Ev, class F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int3,F&&> and has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int3,F&&>)
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int3 grid_shape, F&& function) const
    {
      return bulk_execute_after(std::forward<Ex>(executor), std::forward<Ev>(event), grid_shape, std::forward<F>(function));
    }
    ...
    template<class Ex, class Ev, class F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int4,F&&> and has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int4,F&&>)
    constexpr auto operator()(Ex&& executor, Ev&& event, ubu::int4 grid_shape, F&& function) const
    {
      return bulk_execute_after(std::forward<Ex>(executor), std::forward<Ev>(event), grid_shape, std::forward<F>(function));
    }
  ...
  public:
    ...
    // the following intN overloads exist to allow the user to pass braced lists as the grid_shape parameter
    // like this: bulk_execute_after(ex, before, {x,y,z}, func)
    template<executor Ex, event Ev, std::regular_invocable<ubu::int2> F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int2,F&&> and
                !has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int2,F&&> and
                !std::same_as<executor_coordinate_t<Ex&&>,ubu::int2>)
    constexpr auto operator()(Ex&& executor, Ev&& before, ubu::int2 grid_shape, F&& function) const
    {
      return this->non_native_bulk_execute_after(std::forward<Ex>(executor), std::forward<Ev>(before), grid_shape, std::forward<F>(function));
    }

    template<executor Ex, event Ev, std::regular_invocable<ubu::int3> F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int3,F&&> and
                !has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int3,F&&> and
                !std::same_as<executor_coordinate_t<Ex&&>,ubu::int3>)
    constexpr auto operator()(Ex&& executor, Ev&& before, ubu::int3 grid_shape, F&& function) const
    {
      return this->non_native_bulk_execute_after(std::forward<Ex>(executor), std::forward<Ev>(before), grid_shape, std::forward<F>(function));
    }

    template<executor Ex, event Ev, std::regular_invocable<ubu::int4> F>
      requires (!has_bulk_execute_after_member_function<Ex&&,Ev&&,ubu::int4,F&&> and
                !has_bulk_execute_after_free_function<Ex&&,Ev&&,ubu::int4,F&&> and
                !std::same_as<executor_coordinate_t<Ex&&>,ubu::int4>)
    constexpr auto operator()(Ex&& executor, Ev&& before, ubu::int4 grid_shape, F&& function) const
    {
      return this->non_native_bulk_execute_after(std::forward<Ex>(executor), std::forward<Ev>(before), grid_shape, std::forward<F>(function));
    }
