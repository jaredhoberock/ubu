...
#include "old_bulk_execute_after.hpp"
...
concept can_old_bulk_execute_after = requires(E ex, S shape, F f)
{
  wait(old_bulk_execute_after(ex, first_cause(ex), shape, f));
};


// when old_bulk_execute_after can be called directly with the arguments
template<executor E, coordinate S, std::invocable<S> F>
  requires can_old_bulk_execute_after<E,S&,F&&>
void default_execute_kernel(E ex, const S& shape, F&& f)
{
  // XXX it would be more convenient to call a bulk_execute CPO
  auto finished = old_bulk_execute_after(ex, first_cause(ex), shape, std::forward<F>(f));

  wait(finished);
}


// when old_bulk_execute_after cannot be called directly with the arguments
template<executor E, coordinate S, std::invocable<S> F>
  requires (not can_old_bulk_execute_after<E,S&,F&&>)
void default_execute_kernel(E ex, const S& user_shape, F&& f)
{
  ...
  // XXX it would be more convenient to call a bulk_execute CPO
  auto finished = old_bulk_execute_after(ex, first_cause(ex), shape(to_user_coord), [=](const executor_coordinate_t<E>& ex_coord)
  {
