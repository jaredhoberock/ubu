...
event launch_as_cooperative_kernel_after(const event& before, dim3 grid_dim, dim3 block_dim, std::size_t dynamic_shared_memory_size, cudaStream_t stream, int device, F f)
{
  ...
  temporarily_with_current_device(device, [&]()
  {
    ...
    throw_on_error(cudaStreamWaitEvent(stream, before.native_handle()),
      "cuda::detail::launch_as_cooperative_kernel_after: after cudaStreamWaitEvent"
    );
      ...
      if(grid_dim.x * grid_dim.y * grid_dim.z * block_dim.x * block_dim.y * block_dim.z != 0)
      {
        if UBU_TARGET(ubu::detail::is_host())
        {
          ...
          throw_on_error(cudaLaunchCooperativeKernel(ptr_to_kernel, grid_dim, block_dim, ptr_to_arg, dynamic_shared_memory_size, stream),
            "cuda::detail::launch_as_cooperative_kernel_after: after cudaLaunchCooperativeKernel"
          );
        }
        else
        {
          ubu::detail::throw_runtime_error("cuda::detail::launch_as_cooperative_kernel_after cannot be called from device code.");
        }
      }
    }
    else
    {
      ubu::detail::throw_runtime_error("cuda::detail::launch_as_cooperative_kernel_after requires the CUDA Runtime.");
    }
  });
#else
  ubu::detail::throw_runtime_error("cuda::detail::launch_as_cooperative_kernel_after requires CUDA C++ language support.");
#endif
