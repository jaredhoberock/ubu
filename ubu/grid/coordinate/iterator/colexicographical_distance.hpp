...
constexpr std::integral auto colexicographical_distance(const C1& from, const C2& to, const S& shape)
{
  auto delta = coordinate_difference(to, from);
  return apply_stride(delta, compact_column_major_stride(shape));
}
