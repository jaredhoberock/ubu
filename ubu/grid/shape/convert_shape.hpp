...
// postcondition: grid_size(result) >= grid_size(shape)
//
// if weakly_congruent<T,S>, grid_size(result) == grid_size(shape)
...
constexpr T convert_shape(const S& shape)
{
  return coordinate_cast<T>(grid_size(shape));
}
...
constexpr T convert_shape(const S& shape)
{
  return convert_shape<T>(grid_size(shape));
}
