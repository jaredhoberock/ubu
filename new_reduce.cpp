// ~/Desktop/circle/circle -std=c++20 -I. --cuda-path=/usr/local/cuda -sm_60 --verbose new_reduce.cpp -L/usr/local/cuda/lib64 -lcudart -o a.out
...
ubu::cuda::event reduce_after_at(ubu::cuda::kernel_executor ex, ubu::cuda::device_allocator<int> alloc, const ubu::cuda::event& before, I first, int n, O result, F op)
{
  using namespace ubu;
  ...
  if(num_tiles > 1)
  {
    ...
    auto first_phase = bulk_execute_after(ex, allocation_ready, {num_tiles,block_size}, [=](cuda::thread_id idx)
    {
    ...
    auto second_phase = bulk_execute_after(ex, first_phase, {1,512}, [=](cuda::thread_id idx)
    {
